...
CREATE TABLE IF NOT EXISTS cab (
  ...
  last_update TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (cab_id),
  CONSTRAINT UNIQUE uk_driver_id (driver_id),
  CONSTRAINT fk_cab_driver FOREIGN KEY (driver_id) REFERENCES driver (driver_id) ON DELETE RESTRICT ON UPDATE CASCADE
  );
